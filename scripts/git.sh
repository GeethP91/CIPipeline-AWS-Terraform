...
sudo yum -y install tcpdump bind-utils telnet zip unzip wget -y
...
EFS_FSID_SSH=`/usr/local/bin/aws --region eu-west-1 --output text efs describe-file-system | grep git-ssh | awk '{print $5}'`
EFS_FSID_UPLOADS=`/usr/local/bin/aws --region eu-west-1 --output text efs describe-file-system | grep git-rails-uploads | awk '{print $5}'`
EFS_FSID_SHARED=`/usr/local/bin/aws --region eu-west-1 --output text efs describe-file-system | grep git-rails-shared | awk '{print $5}'`
EFS_FSID_BUILDS=`/usr/local/bin/aws --region eu-west-1 --output text efs describe-file-system | grep git-builds | awk '{print $5}'`
EFS_FSID_DATA=`/usr/local/bin/aws --region eu-west-1 --output text efs describe-file-system | grep git-data | awk '{print $5}'`
...
URL=`aws elbv2 --region eu-west-1 --output text describe-load-balancers| grep private-apps| awk '{print $4}'`;
export URL
