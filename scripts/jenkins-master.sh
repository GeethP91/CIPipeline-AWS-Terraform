#!/bin/bash
sudo yum install -y tcpdump telnet bind-utils wget zip unzip nfs-utils 
...
JENKINS_DIR= "/jenkins"
mkdir -p $JENKINS_DIR
EFS_FSID= `/usr/local/bin/aws --region eu-west-1 --output text efs describe-file-system | grep jenkins | awk '{print $5}'`
